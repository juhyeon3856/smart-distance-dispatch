...
import com.smartdistance.sdd.common.health.dto.KafkaTestRequest;
...
public class HealthController {
    ...
    @Operation(summary = "Kafka 테스트 메시지 전송", description = "테스트 메시지를 Kafka로 전송")
    @PostMapping("/kafka/test")
    public ResponseEntity<KafkaTestResponse> sendTestMessage(
            @RequestBody KafkaTestRequest request)
    {

        KafkaTestResponse response = kafkaHealthCheckService.sendTestMessage(request.getMessage());
